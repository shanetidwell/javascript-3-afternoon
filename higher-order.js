...
//Code Here
let totalPopulation   = populations.reduce((arr,num)=>arr+num)
...
//Code Here
let myStrongest  = monstersInYourPocket.filter((obj, index, array)=>{
  return obj.CP>200;
})
...
let ordersTotal = orders.map((obj)=>{
  return {"totals":(obj.price + obj.tax)};
});//Code Here
...
*/
let bobsTotal = purchases.reduce((accum,obj,index,array)=>{
  if(array[index].owner === "Bob"){ 
      return accum + array[index].price;
  }
  else{
      return accum
  }
